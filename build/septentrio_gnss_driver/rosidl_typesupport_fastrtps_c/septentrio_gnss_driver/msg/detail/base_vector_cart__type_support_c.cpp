  ...
  void * untyped_ros_message)
{
  ...
  // Field name: vector_info_cart
  {
    ...
    if (!septentrio_gnss_driver__msg__VectorInfoCart__Sequence__init(&ros_message->vector_info_cart, size)) {
      return "failed to create array for field 'vector_info_cart'";
    }
    auto array_ptr = ros_message->vector_info_cart.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _BaseVectorCart__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_septentrio_gnss_driver__msg__BaseVectorCart(
    full_bounded, 0);
}
