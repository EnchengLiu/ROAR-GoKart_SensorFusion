  ...
  septentrio_gnss_driver__msg__PVTCartesian__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    septentrio_gnss_driver__msg__PVTCartesian * data =
      (septentrio_gnss_driver__msg__PVTCartesian *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!septentrio_gnss_driver__msg__PVTCartesian__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          septentrio_gnss_driver__msg__PVTCartesian__fini(&data[i]);
        }
        free(data);
        return false;
      }
