  ...
  septentrio_gnss_driver__msg__VelCovGeodetic__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    septentrio_gnss_driver__msg__VelCovGeodetic * data =
      (septentrio_gnss_driver__msg__VelCovGeodetic *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!septentrio_gnss_driver__msg__VelCovGeodetic__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          septentrio_gnss_driver__msg__VelCovGeodetic__fini(&data[i]);
        }
        free(data);
        return false;
      }
