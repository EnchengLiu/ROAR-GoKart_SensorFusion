  ...
  void * untyped_ros_message)
{
  ...
  // Field name: rfband
  {
    ...
    if (!septentrio_gnss_driver__msg__RFBand__Sequence__init(&ros_message->rfband, size)) {
      return "failed to create array for field 'rfband'";
    }
    auto array_ptr = ros_message->rfband.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _RFStatus__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_septentrio_gnss_driver__msg__RFStatus(
    full_bounded, 0);
}
