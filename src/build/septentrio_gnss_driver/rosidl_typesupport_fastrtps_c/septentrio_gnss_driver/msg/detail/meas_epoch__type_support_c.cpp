  ...
  void * untyped_ros_message)
{
  ...
  // Field name: type1
  {
    ...
    if (!septentrio_gnss_driver__msg__MeasEpochChannelType1__Sequence__init(&ros_message->type1, size)) {
      return "failed to create array for field 'type1'";
    }
    auto array_ptr = ros_message->type1.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _MeasEpoch__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_septentrio_gnss_driver__msg__MeasEpoch(
    full_bounded, 0);
}
