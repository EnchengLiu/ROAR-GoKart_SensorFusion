...
namespace ROAR
{
  namespace SeptentrioInterface
  {
    SeptentrioInterfaceNode::SeptentrioInterfaceNode() : rclcpp::Node("septentrio_interface_node")
    {
      ...
      this->path_publisher_ = this->create_publisher<nav_msgs::msg::Path>("/roar/path", 10);
    ...
    void SeptentrioInterfaceNode::gps_callback(const gps_msgs::msg::GPSFix::SharedPtr msg)
    {
      ...
      path.header.frame_id = this->get_parameter("map_frame").as_string();
      ...
      RCLCPP_DEBUG_STREAM(get_logger(), "\n-------------"
                                            ...
                                            << "\nLastPath: " << path.poses.back().pose.position.x << ", " << path.poses.back().pose.position.y << ", " << path.poses.back().pose.position.z
